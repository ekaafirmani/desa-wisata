...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Sewa Gasebo Besar</h2>
        <div class="text-right">
            <p class="text-xs text-gray-500">Total Hari Ini</p>
            <p class="text-lg font-bold text-amber-700">Rp {{ number_format($totalHariIni, 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow p-6 lg:col-span-1">
            ...
            <form method="POST" action="{{ route('kuliner.gasebo.simpan') }}" class="space-y-4">
                @csrf
                <div>
                    ...
                    <select name="gasebo_id" id="gasebo_id" onchange="hitungTotalGasebo()" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-600">
                ...
                <div>
                    ...
                    <input type="number" name="durasi_jam" id="durasi_jam" min="3" value="{{ old('durasi_jam', 3) }}"
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="2"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-600">{{ old('catatan') }}</textarea>
                </div>

                {{-- Total Bayar --}}
                <div class="bg-amber-50 rounded-lg px-4 py-3">
                    <p class="text-xs text-gray-500">Total Bayar</p>
                    <p class="text-xl font-bold text-amber-700" id="total_bayar_display">Rp 0</p>
                </div>
                            ...
                            @forelse ($sewaAktif as $s)
                                <tr>
                                    ...
                                    <td class="px-3 py-2 text-right">
                                        <form method="POST" action="{{ route('kuliner.gasebo.selesai', $s->id) }}"
                                              onsubmit="return confirm('Tandai sewa gasebo ini selesai?')">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-amber-700 hover:underline text-xs font-medium">
                                                Tandai Selesai
                                            </button>
                                        </form>
                                    </td>
    ...
    <script>
        function hitungTotalGasebo() {
            ...
            const durasiMin = selected ? (parseInt(selected.getAttribute('data-min')) || 3) : 3;
