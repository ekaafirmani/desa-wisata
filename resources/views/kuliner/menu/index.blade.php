...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Kelola Menu Kuliner</h2>
        <a href="{{ route('kuliner.menu.tambah') }}"
                ...
                @forelse ($menu as $m)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-right space-x-3">
                            <a href="{{ route('kuliner.menu.edit', $m->id) }}"
                               class="text-blue-600 hover:underline text-xs">
                                Edit
                            </a>

                            @if ($m->tersedia)
                                <form method="POST" action="{{ route('kuliner.menu.nonaktifkan', $m->id) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            @else
                                <form method="POST" action="{{ route('kuliner.menu.aktifkan', $m->id) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
