    ...
    <div class="bg-white rounded-xl shadow p-6 max-w-lg">
        <form method="POST" action="{{ route('admin.petugas.update', $petugas->id) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="role"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600">
                    <option value="loket" {{ old('role', $petugas->role) == 'loket' ? 'selected' : '' }}>Loket</option>
                    <option value="tubing_mini" {{ old('role', $petugas->role) == 'tubing_mini' ? 'selected' : '' }}>Tubing Mini</option>
                    <option value="tubing_dewasa" {{ old('role', $petugas->role) == 'tubing_dewasa' ? 'selected' : '' }}>Tubing Dewasa</option>
                    <option value="kolam" {{ old('role', $petugas->role) == 'kolam' ? 'selected' : '' }}>Kolam</option>
                    <option value="kuliner" {{ old('role', $petugas->role) == 'kuliner' ? 'selected' : '' }}>Kuliner</option>
                </select>
                @error('role')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
