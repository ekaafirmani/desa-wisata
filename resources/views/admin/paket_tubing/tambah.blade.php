...
@section('title', 'Tambah Paket Tubing')

@section('content')

    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Tambah Paket Tubing</h2>
    </div>

    <div class="bg-white rounded-xl shadow p-6 max-w-lg">
        <form method="POST" action="{{ route('admin.paket_tubing.simpan') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="jenis"
                    ...
                    <option value="mini" {{ old('jenis') == 'mini' ? 'selected' : '' }}>Mini</option>
                    <option value="dewasa" {{ old('jenis') == 'dewasa' ? 'selected' : '' }}>Dewasa</option>
                </select>
                @error('jenis')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="fasilitas" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600">{{ old('fasilitas') }}</textarea>
                @error('fasilitas')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
